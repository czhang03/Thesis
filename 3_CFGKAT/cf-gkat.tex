...
\begin{definition}[Intermediate semantics, base]
...
First of all, the $\comITE{b}{e}{f}$ filters out traces in the semantics of the $e$ that satisfy the guard $b$, as well as the traces in $f$ that invalidate it.
 ...
 \begin{definition}[Intermediate semantics, branching]
 Let $e, f \in \CFGKAT$.
 ...
 \begin{definition}[Intermediate semantics, sequencing and loops]%
...
\begin{definition}[Intermediate semantics starting from a label, base]
 Let $e ∈ \CFGKAT$.
...
\begin{definition}[Intermediate semantics starting from a label, sequencing and branching]
...
\begin{definition}[Intermediate semantics starting from a label, loops]
...
Crucially, we can retrofit the intermediate semantics $⟦ e ⟧$ as a labeled family; after all, $⟦ e ⟧^\sharp$ is an indexed family, and so is $⟦ e ⟧^\ell$ for each $\ell \in L$.
