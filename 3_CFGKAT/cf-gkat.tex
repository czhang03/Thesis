...
\begin{definition}
 ...
 \begin{align*}
  ⟦ \false ⟧ & ≜ ∅
    & ⟦ t ⟧& ≜ \{ (α, i) ∣ t ∈ α, i ∈ I \}
    & ⟦ e_{b1} ∨ e_{b2} ⟧ & ≜ ⟦ e_{b1} ⟧ ∪ ⟦ e_{b2} ⟧ \\
  ⟦ \true ⟧  & ≜ \At × I
    & ⟦ x = i ⟧ & ≜ \{ (α, i) ∣ α ∈ \At \}
    & ⟦ e_{b1} ∧ e_{b2} ⟧ & ≜ ⟦ e_{b1} ⟧ ∩ ⟦ e_{b2} ⟧ \\
  & & & & ⟦ ¬ e_{b} ⟧ & ≜ \At × I \setminus ⟦ e_b ⟧
 \end{align*}
\end{definition}
...
\begin{definition}[continuation semantics, base]
 ...
 \begin{align*}
  ⟦\comAssert{e_b}⟧ᵢ^♯ & ≜ \{ α ⋅ \acc{i} ∣ (α, i) ∈ ⟦ e_b ⟧ \}
...
First of all, the $\comITE{b}{e}{f}$ filters out traces in the semantics of the $e$ that satisfy the guard $b$, as well as the traces in $f$ that invalidate it.
 ...
 \(
    ⟦e; f⟧^♯ ≜ ⟦ e ⟧^♯ ⋄ ⟦ f ⟧^♯
 \).
 Also, for all $e_b ∈ \BExp$, we define $⟦\comWhile{e_b}{e}⟧^♯$ as the least indexed family satisfying:
 \begin{mathpar}
    \inferrule{%
        i ∈ I \\
        α ∉ ⟦ e_b ⟧
    }{%
        α ⋅ \acc{i} ∈ ⟦\comWhile{e_b}{e}⟧^♯ᵢ
    }
    \and
    \inferrule{%
        α ∈ ⟦ e_b ⟧ \\
        α w ⋅ c ∈ (⟦ e ⟧^♯ ⋄ ⟦\comWhile{e_b}{e}⟧^♯)ᵢ
    }{%
        α w ⋅ ⌊ c ⌋ ∈ ⟦\comWhile{e_b}{e}⟧^♯_i
    }
 \end{mathpar}
...
\begin{mathpar}
    ⟦\comITE{e_b}{e}{f}⟧^ℓᵢ ≜ ⟦ e ⟧^ℓᵢ \cup ⟦ f ⟧^ℓᵢ
    \and
...
\[
    ⟦\comWhile{e_b}{e}⟧^ℓ_i = \{ w \cdot \lfloor c \rfloor \mid w \cdot c \in (⟦ e ⟧^ℓ ⋄ ⟦\comWhile{e_b}{e}⟧^♯)ᵢ \}
\]
\end{definition}
...
Crucially, we can retrofit the continuation semantics $⟦ e ⟧$ as a labeled family; after all, $⟦ e ⟧^♯$ is an indexed family, and so is $⟦ e ⟧^ℓ$ for each $ℓ ∈ L$.
...
\begin{definition}[GKAT automata~\cite{Schmid_Kappé_Kozen_Silva_2021, kozen_BohmJacopiniTheorem_2008a}]
 A \emph{GKAT automaton} $A ≜ ⟨ S, δ, \hat{s} ⟩$ consists of a set of \emph{states} $S$, a \emph{transition function} $\delta: S → \At → ⊥ + ⊤ + K × S$, and a \emph{start state} $\hat{s} ∈ S$.
\end{definition}
Intuitively, the transition function of a GKAT automaton tells us, given a state $q$ and an atom $α$ accounting for the truth value of each primitive test, to either \emph{reject} the input, represented by $δ(q, α) = ⊥$, \emph{accept} the input, represented by $δ(q, α) = ⊤$, or to \emph{transition} to a new state in $S$ after executing an action from $K$, represented by $δ(q, α) ∈ K × S$.
...
\begin{definition}[Lowering CF-GKAT automata]
 ...
 \end{align*}
\end{definition}
Notice \(λ\!↓\) is total and \(δ'((s, i), α)\) will not return a jump continuation, therefore $δ\!↓$ is well-defined; in other words, for all $s ∈ S$, $i ∈ I$ and $α ∈ \At$, it holds that $δ\!↓((s, i), α) ∈ ⊥ + ⊤ + K × (S × I)$, as expected for a GKAT automaton on state set $S × I$.
...
\begin{table}
  ...
  \begin{tabular}{c || c | l | l | l}
   \(e\)  & \(S\) & \(δ(s): G(S)\) & \(ι(i, α): ⊥ + C + K × S × I\) & \(λ(ℓ): G(S)\) \\
   \hline
   \(\comAssert{e_b}\)& \(∅\)& \(!\)& \(
   \begin{cases}
    \acc{i} & (α, i) ∈ ⟦ e_b ⟧   \\
