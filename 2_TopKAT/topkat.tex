...
\end{definition}
Note that the original definition of \citet{Mamouras_2017} allows for
try-catch statements and different types of errors.  We omit these features for
simplicity, since they are not needed in incorrectness
logic~\cite{OHearn_2020}. For a more explicit definition
of \(FailTopKAT\) with all the rules, please refer to the
\hyperref[def: explicit fail top kat]{definition} on page~\pageref{def: explicit fail top kat}. 
...
this triple can be automatically decided using the algorithm in \Cref{the: PSPACE for TopKAT equalities}.
\end{example}     
